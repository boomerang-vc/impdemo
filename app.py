...
def create_overhead_foot_tracking_visualization(current_step: int = 0, num_steps: int = 10, target_step_length: float = 0.75):
    ...
    fig = go.Figure()
    fig.add_shape(type="rect", x0=-0.4, y0=0, x1=0.4, y1=num_steps * target_step_length + 1,
                  line=dict(color="gray", width=2), fillcolor="lightgray", opacity=0.2, layer="below")
    fig.add_annotation(x=0, y=num_steps * target_step_length + 0.5, text="↑ Direction of Movement",
                       showarrow=False, font=dict(size=14, color="gray"), opacity=0.7)
    color_map = {"Right": "#FF6B6B", "Left": "#4ECDC4"}
    for idx, row in df_track.iterrows():
        if idx == current_step:
            size, opacity, line_width = 35, 1.0, 4
        elif idx < current_step:
            size, opacity, line_width = 22, 0.6, 2
        else:
            size, opacity, line_width = 18, 0.3, 1
        foot_symbol = "triangle-right" if row['foot'] == "Right" else "triangle-left"
        fig.add_trace(go.Scatter(
            x=[row["x"]], y=[row["y"]], mode="markers+text",
            marker=dict(size=size, color=color_map[row["foot"]], opacity=opacity,
                        symbol=foot_symbol, line=dict(width=line_width, color=color_map[row["foot"]])),
            text=[f"S{row['step']}"], textposition="middle center",
            textfont=dict(size=10, color="white", family="Arial Black"),
            name=f"{row['foot']} Foot",
            hovertemplate=(f"<b>{row['foot']} Foot</b><br>Step: {row['step']}<br>"
                           f"Position: {row['y']:.2f}m<br>Lateral: {row['x']:.2f}m<br><extra></extra>"),
            showlegend=(idx in [0, 1]),
        ))
        target_y = (idx + 1) * target_step_length
        if idx < num_steps - 1:
            fig.add_shape(type="line", x0=-0.35, y0=target_y, x1=0.35, y1=target_y,
                          line=dict(color="green", width=1, dash="dot"), opacity=0.3, layer="below")
    fig.update_layout(
        title=dict(text=f"<b>Stepping Target (Current Step: {current_step})</b>",
                   font=dict(size=20, color="#d32f2f")),
        xaxis=dict(title="Left ← | → Right (meters)", range=[-0.5, 0.5], showgrid=True,
                   gridcolor="rgba(0,0,0,0.1)", zeroline=True, zerolinewidth=2, zerolinecolor="black"),
        yaxis=dict(title="Forward Movement (meters)", range=[-0.5, num_steps * target_step_length + 1],
                   showgrid=True, gridcolor="rgba(0,0,0,0.1)"),
        height=600, plot_bgcolor="rgba(250,250,250,1)", paper_bgcolor="white",
...
def create_hero_summary(stats, session_time, distance, user_mode="Administrator"):
    header_text = "Great Work! Here's Your Summary" if user_mode == "Patient" else "Session Complete!"
    hero_html = f"""
    <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
                padding: 30px; border-radius: 15px; color: white; text-align: center;
                box-shadow: 0 10px 30px rgba(0,0,0,0.3);">
        <h1 style="margin: 0; font-size: 2.5em; font-weight: bold;">{header_text}</h1>
...
def create_real_time_symmetry_bars(left_value, right_value, metric_name, target_min=None, target_max=None):
    ...
    fig.add_trace(go.Bar(x=[left_value], y=['Left'], orientation='h', name='Left',
                         marker=dict(color=left_color), text=[f'{left_value:.2f}'], textposition='auto'))
    fig.add_trace(go.Bar(x=[right_value], y=['Right'], orientation='h', name='Right',
                         marker=dict(color=right_color), text=[f'{right_value:.2f}'], textposition='auto'))
    if target_min and target_max:
        fig.add_vrect(x0=target_min, x1=target_max, fillcolor="green", opacity=0.1, layer="below", line_width=0)
    fig.update_layout(title=f"Real-Time Symmetry: {metric_name}", xaxis_title=metric_name,
                      height=200, showlegend=False, margin=dict(l=80, r=40, t=60, b=40), plot_bgcolor="white")
    return fig
...
def initialize_session_state():
    defaults = {
        'user_mode': None, 'logged_in': False, 'user_email': "",
        'patient_profiles': {}, 'session_history': [], 'favorite_metrics': [],
        'target_zones': {}, 'self_pacing_enabled': False
    }
...
def apply_admin_styles():
    st.markdown("""
    <style>
        .main-header { font-size: 2.5em; color: #d32f2f; text-align: center; margin-bottom: 1rem; font-weight: bold; }
...
def apply_patient_styles():
    st.markdown("""
    <style>
        .main-header { font-size: 2.8em; color: #1b5e20; text-align: center; margin-bottom: 1rem; font-weight: bold; letter-spacing: 1px; }
        .sub-header { font-size: 2rem; color: #2e7d32; margin-top: 2rem; margin-bottom: 1rem; }
        .metric-card { background-color: #e8f5e9; border-radius: 15px; padding: 20px; box-shadow: 3px 3px 8px rgba(0,100,0,0.15); }
        .large-metric { font-size: 5em; font-weight: bold; text-align: center; padding: 35px; border-radius: 20px; }
        .medium-metric { font-size: 2.5em; font-weight: bold; text-align: center; padding: 25px; border-radius: 15px; }
        .status-green { background-color: #2e7d32; color: white; }
        .status-yellow { background-color: #f9a825; color: black; }
        .status-red { background-color: #b71c1c; color: white; }
        .status-gray { background-color: #546e7a; color: white; }
        .stButton>button { width: 100%; height: 80px; font-size: 1.5em; border-radius: 12px; }
...
def _render_progress_tracking(user_label="Administrator"):
    color = "#d32f2f" if user_label == "Administrator" else "#1b5e20"
    if st.session_state.session_history:
        st.subheader("Session History")
        history_df = pd.DataFrame([
            {
                'Date': s['date'],
                'Duration (min)': f"{s['duration']:.1f}",
                'Distance (m)': f"{s['distance']:.1f}",
                'Avg Speed (mph)': f"{s['avg_speed']:.2f}",
        ...
        if len(st.session_state.session_history) > 1:
            ...
            mapping = {
                "Avg Speed": ("avg_speed", "Speed (mph)"),
                "Avg Cadence": ("avg_cadence", "Cadence (steps/min)"),
                "Distance": ("distance", "Distance (m)"),
                "Duration": ("duration", "Duration (min)")
            }
            key, ylabel = mapping[trend_metric]
            values = [s[key] for s in st.session_state.session_history]
            fig_trend = go.Figure()
            fig_trend.add_trace(go.Scatter(x=dates, y=values, mode='lines+markers', name=trend_metric,
                                           line=dict(width=3, color=color), marker=dict(size=10)))
            fig_trend.update_layout(title=f"{trend_metric} Over Time", xaxis_title="Session Date",
        ...
        with pr_col1:
            st.metric("Fastest Speed 🏃", f"{max_speed_session['avg_speed']:.2f} mph", delta=max_speed_session['date'])
        with pr_col2:
            st.metric("Longest Distance 📏", f"{max_distance_session['distance']:.0f} m", delta=max_distance_session['date'])
        with pr_col3:
            st.metric("Longest Session ⏱️", f"{max_duration_session['duration']:.1f} min", delta=max_duration_session['date'])
        st.markdown("---")
        ...
        st.info("No session history yet. Complete a session to start tracking!")


def _render_stepping_target(admin_controlled=True):
    track_col1, track_col2 = st.columns([3, 1])
    with track_col1:
        if admin_controlled:
            target_step_length = st.slider(
                "Target Step Length (meters) — Set by Administrator",
                min_value=0.5, max_value=1.0, value=0.75, step=0.05)
        else:
            target_step_length = st.slider(
                "Target Step Length (meters)",
                min_value=0.5, max_value=1.0, value=0.75, step=0.05,
                help="Try to match this step length during your session")
        ...
        fig_overhead = create_overhead_foot_tracking_visualization(
            current_step=current_step, num_steps=10, target_step_length=target_step_length)
        st.plotly_chart(fig_overhead, use_container_width=True)
    with track_col2:
        st.subheader("Stats")
        st.metric("Current Step", current_step)
        st.metric("Total Steps", 10)
        st.metric("On Target", f"{np.random.randint(75, 95)}%")
        st.metric("Score", np.random.randint(80, 100))
        st.markdown("---")
    ...
    st.subheader("Accuracy Score Over Session")
    fig_target_perf = go.Figure()
    accuracy_scores = [np.random.randint(70, 100) for _ in range(10)]
    fig_target_perf.add_trace(go.Scatter(x=list(range(10)), y=accuracy_scores, mode='lines+markers',
                                          name='Accuracy Score', line=dict(color='#28a745', width=3), marker=dict(size=10)))
    fig_target_perf.add_hline(y=90, line_dash="dash", line_color="green", annotation_text="Target: 90%")
    fig_target_perf.update_layout(title="Step Accuracy Score", xaxis_title="Step Number",
                                   yaxis_title="Accuracy Score (%)", height=400, yaxis_range=[0, 100])
    st.plotly_chart(fig_target_perf, use_container_width=True)


def _render_export_section(stats, total_distance, filtered_df, user_mode="Administrator"):
    title_color = "#d32f2f" if user_mode == "Administrator" else "#1b5e20"
    st.markdown("---")
    st.markdown(f'<span style="color:{title_color}; font-size:28px;">Export Options</span>', unsafe_allow_html=True)
    ...
    with export_col1:
        if st.button("Export Statistics Summary"):
            summary_stats = pd.DataFrame({
                ...
                'Value': [
                    f"{stats['avg_speed']:.2f}", f"{stats['avg_right_cadence']:.1f}", f"{stats['avg_left_cadence']:.1f}",
                    f"{stats['avg_right_step_length']:.3f}", f"{stats['avg_left_step_length']:.3f}",
                    f"{stats['avg_right_stride_length']:.3f}", f"{stats['avg_left_stride_length']:.3f}",
                    f"{stats['right_swing_stance_ratio']:.2f}", f"{stats['left_swing_stance_ratio']:.2f}",
                    f"{stats['temporal_asymmetry']:.1f}", f"{stats['spatial_asymmetry']:.1f}",
                ]
            })
            csv_stats = summary_stats.to_csv(index=False)
            pid = st.session_state.get('patient_id', 'user')
            st.download_button("Download Statistics CSV", data=csv_stats,
                               file_name=f"gait_stats_{pid}.csv", mime="text/csv")
    ...
    with export_col2:
        def export_full_report_to_pdf(stats, patient_id):
            ...
            label = "Patient Gait Analysis Report" if user_mode == "Administrator" else "My Gait Analysis Report"
        ...
        if st.button("Export Full Report"):
            pid = st.session_state.get('patient_id', 'user')
            pdf_buffer = export_full_report_to_pdf(stats, pid)
            st.download_button("Download Full Report as PDF", data=pdf_buffer,
                               file_name=f"gait_report_{pid}.pdf", mime="application/pdf")
    st.markdown(f'<span style="color:{title_color}; font-size:28px;">Raw Data</span>', unsafe_allow_html=True)
    if st.checkbox("Show Raw Data Table"):
        st.dataframe(filtered_df)
        if st.button("Export Raw Data"):
            csv_data = filtered_df.to_csv(index=False)
            pid = st.session_state.get('patient_id', 'user')
            st.download_button("Download Raw Data CSV", data=csv_data,
                               file_name=f"raw_gait_data_{pid}.csv", mime="text/csv")


# ─── ADMINISTRATOR DASHBOARD ─────────────────────────────────────────────────

def render_admin_dashboard(df, stats, session_time, total_distance,
                            target_cadence_min, target_cadence_max,
                            target_step_length_min, target_step_length_max,
                            filtered_df):
    tab1, tab2, tab3, tab4, tab5 = st.tabs([
        "Real-Time Display", "Hero Summary", "Post-Analysis",
        "Progress Tracking", "Stepping Target"
    ])
    ...
    with tab1:
        ...
        with col1:
            ...
            if 'cadence' in st.session_state.target_zones:
                t_min, t_max = st.session_state.target_zones['cadence']
                if t_min <= cadence_value <= t_max:
                    status_class = "status-green"
                elif (t_min * 0.9 <= cadence_value < t_min) or (t_max < cadence_value <= t_max * 1.1):
    ...
    with tab2:
        ...
        st.markdown(create_hero_summary(stats, session_time, total_distance, "Administrator"), unsafe_allow_html=True)
        st.markdown("---")
        ...
        if st.button("Save Session"):
            st.session_state.session_history.append({
                ...
                'duration': session_time, 'distance': total_distance,
                'avg_speed': stats['avg_speed'], 'avg_cadence': stats['avg_right_cadence'],
                'notes': session_notes, 'stats': stats
            })
    ...
    with tab3:
        ...
        with analysis_tabs[0]:
            ...
            fig.update_layout(title=f"{metric_choice} Over Session", xaxis_title="Time (samples)",
                              yaxis_title=metric_choice, height=500, hovermode='x unified')
        ...
        with analysis_tabs[1]:
            ...
            st.plotly_chart(create_fatigue_analysis(filtered_df, fatigue_metric), use_container_width=True)
            st.info("""
        ...
        with analysis_tabs[2]:
            ...
            if len(st.session_state.session_history) > 1:
                ...
                fig_comp = go.Figure()
                fig_comp.add_trace(go.Scatter(x=comparison_df['Session'], y=comparison_df['Avg Cadence'],
                                              mode='lines+markers', name='Average Cadence', line=dict(width=3)))
                fig_comp.update_layout(title="Cadence Progress Over Sessions",
                                       xaxis_title="Session Number", yaxis_title="Average Cadence (steps/min)", height=400)
        ...
        with analysis_tabs[3]:
            ...
            fig_sym = go.Figure()
            fig_sym.add_trace(go.Scatter(x=filtered_df['Sample Index'], y=filtered_df['Temporal_Asymmetry'],
                                         mode='lines', name='Temporal Asymmetry', line=dict(color='#FF6B6B', width=2)))
            fig_sym.add_trace(go.Scatter(x=filtered_df['Sample Index'], y=filtered_df['Spatial_Asymmetry'],
                                         mode='lines', name='Spatial Asymmetry', line=dict(color='#4ECDC4', width=2)))
            fig_sym.add_hline(y=10, line_dash="dash", line_color="orange", annotation_text="10% Threshold")
            fig_sym.update_layout(title="Asymmetry Percentage Over Time", xaxis_title="Time (samples)",
                                  yaxis_title="Asymmetry (%)", height=500, hovermode='x unified')
            st.plotly_chart(fig_sym, use_container_width=True)

    with tab4:
        st.markdown('<h2 style="color:#d32f2f;">Progress Tracking</h2>', unsafe_allow_html=True)
        _render_progress_tracking(user_label="Administrator")

    with tab5:
        st.markdown('<h2 style="color:#d32f2f;">Stepping Target</h2>', unsafe_allow_html=True)
        _render_stepping_target(admin_controlled=True)

    _render_export_section(stats, total_distance, filtered_df, user_mode="Administrator")


# ─── PATIENT DASHBOARD ───────────────────────────────────────────────────────

def render_patient_dashboard(df, stats, session_time, total_distance,
                              target_cadence_min, target_cadence_max,
                              target_step_length_min, target_step_length_max,
                              filtered_df):
    tab1, tab2, tab3, tab4, tab5 = st.tabs([
        "🏃 Live Metrics", "🏅 My Results", "📊 Analysis",
        "📈 My Progress", "👣 Step Guide"
    ])

    with tab1:
        st.markdown('<h2 style="color:#1b5e20; font-size:2em;">Your Live Performance</h2>', unsafe_allow_html=True)
        st.caption("Glance at your key numbers below — updated in real time.")
        col1, col2, col3 = st.columns([2, 1, 1])
        with col1:
            cadence_value = stats['avg_right_cadence']
            if 'cadence' in st.session_state.target_zones:
            ...
            st.markdown(f'''
                <div class="large-metric {status_class}">
                    <div style="font-size:0.3em;opacity:0.85;letter-spacing:2px;">STEPS PER MINUTE</div>
                    <div>{cadence_value:.0f}</div>
                    <div style="font-size:0.35em;opacity:0.8;">Your Rhythm (Cadence)</div>
                </div>''', unsafe_allow_html=True)
        with col2:
            st.markdown(f'''
                <div class="medium-metric status-green">
                    <div style="font-size:0.4em;opacity:0.85;">YOUR SPEED</div>
                    <div>{stats['avg_speed']:.1f}</div>
                    <div style="font-size:0.5em;">mph</div>
                </div>''', unsafe_allow_html=True)
        with col3:
            st.markdown(f'''
                <div class="medium-metric status-green">
                    <div style="font-size:0.4em;opacity:0.85;">STEP LENGTH</div>
                    <div>{stats['avg_right_step_length']:.2f}</div>
                    <div style="font-size:0.5em;">meters</div>
                </div>''', unsafe_allow_html=True)
        st.markdown("---")
        st.markdown("### ⚖️ Left vs Right Balance")
        st.caption("These bars show how evenly you are stepping with each foot.")
        sym_col1, sym_col2 = st.columns(2)
        with sym_col1:
            fig_sym_cadence = create_real_time_symmetry_bars(
                stats['avg_left_cadence'], stats['avg_right_cadence'], "Steps per Minute",
                target_cadence_min if 'cadence' in st.session_state.target_zones else None,
                target_cadence_max if 'cadence' in st.session_state.target_zones else None)
            st.plotly_chart(fig_sym_cadence, use_container_width=True)
        with sym_col2:
            fig_sym_step = create_real_time_symmetry_bars(
                stats['avg_left_step_length'], stats['avg_right_step_length'], "Step Length (m)",
                target_step_length_min if 'step_length' in st.session_state.target_zones else None,
                target_step_length_max if 'step_length' in st.session_state.target_zones else None)
            st.plotly_chart(fig_sym_step, use_container_width=True)
        st.markdown("---")
        ...
        asym_col1, asym_col2 = st.columns(2)
        with asym_col1:
            st.metric("Timing Balance", f"{stats['temporal_asymmetry']:.1f}% difference",
                      delta="Great!" if stats['temporal_asymmetry'] < 10 else "Keep working on it",
                      delta_color="inverse" if stats['temporal_asymmetry'] < 10 else "normal",
                      help="How evenly timed your left and right steps are")
        with asym_col2:
            st.metric("Step Length Balance", f"{stats['spatial_asymmetry']:.1f}% difference",
                      delta="Great!" if stats['spatial_asymmetry'] < 10 else "Keep working on it",
                      delta_color="inverse" if stats['spatial_asymmetry'] < 10 else "normal",
                      help="How evenly sized your left and right steps are")

    with tab2:
        st.markdown('<h2 style="color:#1b5e20;">Great Work! Here\'s Your Summary</h2>', unsafe_allow_html=True)
        st.markdown(create_hero_summary(stats, session_time, total_distance, "Patient"), unsafe_allow_html=True)
        st.markdown("---")
        st.subheader("Session Notes")
        session_notes = st.text_area("How did this session feel? Add any notes:", height=120)
        if st.button("💾 Save My Session"):
            st.session_state.session_history.append({
                'date': datetime.now().strftime("%Y-%m-%d %H:%M"),
            ...
            st.success("✅ Session saved! Keep it up!")
        st.markdown("---")
        st.subheader("📤 Share Your Report")
        if st.button("📧 Email Me a PDF Report"):
            st.success(f"✅ Report sent to {st.session_state.user_email}!")

    with tab3:
        st.markdown('<h2 style="color:#1b5e20;">Detailed Analysis</h2>', unsafe_allow_html=True)
        analysis_tabs = st.tabs(["📉 Trends", "🔋 Energy & Fatigue", "🔁 Compare Sessions", "⚖️ Symmetry"])
        with analysis_tabs[0]:
            st.subheader("Your Metrics Over Time")
            metric_choice = st.selectbox("Which metric would you like to explore?",
                ["Right Cadence (steps/min)", "Left Cadence (steps/min)",
                 "Right Step Length (meters)", "Left Step Length (meters)", "GaitSpeed Rtable (mph*10)"])
            filtered_df['Sample Index'] = range(len(filtered_df))
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=filtered_df['Sample Index'], y=filtered_df[metric_choice],
                                     mode='lines+markers', name=metric_choice, line=dict(width=3, color="#2e7d32")))
            if st.session_state.self_pacing_enabled:
                fig.add_vline(x=len(filtered_df)//2, line_dash="dash", line_color="green",
                              annotation_text="Self-Pacing On", annotation_position="top")
            fig.update_layout(title=f"{metric_choice} Through Your Session",
                              xaxis_title="Time", yaxis_title=metric_choice, height=450, hovermode='x unified')
            st.plotly_chart(fig, use_container_width=True)
        with analysis_tabs[1]:
            st.subheader("Did Your Performance Change?")
            st.caption("Compares how you performed at the start vs. the end of the session.")
            fatigue_metric = st.selectbox("Choose a metric:",
                ["Right Cadence (steps/min)", "Left Cadence (steps/min)",
                 "Right Step Length (meters)", "Left Step Length (meters)"], key="patient_fatigue_metric")
            st.plotly_chart(create_fatigue_analysis(filtered_df, fatigue_metric), use_container_width=True)
        with analysis_tabs[2]:
            st.subheader("How Do Your Sessions Compare?")
            if len(st.session_state.session_history) > 1:
                comparison_df = pd.DataFrame([
                    {'Session': i+1, 'Date': s['date'], 'Avg Speed': s['avg_speed'],
                     'Avg Cadence': s['avg_cadence'], 'Distance': s['distance']}
                ...
                fig_comp.add_trace(go.Scatter(x=comparison_df['Session'], y=comparison_df['Avg Cadence'],
                                              ...
                                              line=dict(width=3, color="#2e7d32"), marker=dict(size=10)))
                fig_comp.update_layout(title="Your Cadence Across Sessions",
                                       xaxis_title="Session Number", yaxis_title="Steps per Minute", height=400)
                st.plotly_chart(fig_comp, use_container_width=True)
            else:
                st.info("Complete more sessions to compare your progress!")
        with analysis_tabs[3]:
            st.subheader("Your Balance Over Time")
            filtered_df['Temporal_Asymmetry'] = filtered_df.apply(
                lambda row: calculate_asymmetry_percentage(row['Left Step Time (sec)'], row['Right Step Time (sec)']), axis=1)
            filtered_df['Spatial_Asymmetry'] = filtered_df.apply(
                lambda row: calculate_asymmetry_percentage(row['Left Step Length (meters)'], row['Right Step Length (meters)']), axis=1)
            ...
            fig_sym.add_hline(y=10, line_dash="dash", line_color="orange", annotation_text="10% = Good Balance Goal")
            fig_sym.update_layout(title="Balance Percentage — Lower is Better",
                                  xaxis_title="Time", yaxis_title="Difference (%)", height=450, hovermode='x unified')
            st.plotly_chart(fig_sym, use_container_width=True)

    with tab4:
        st.markdown('<h2 style="color:#1b5e20;">My Progress</h2>', unsafe_allow_html=True)
        _render_progress_tracking(user_label="Patient")

    with tab5:
        st.markdown('<h2 style="color:#1b5e20;">Step Guide</h2>', unsafe_allow_html=True)
        st.caption("Follow the footstep guide below and try to land as close to the target lines as possible!")
        _render_stepping_target(admin_controlled=False)

    _render_export_section(stats, total_distance, filtered_df, user_mode="Patient")


# ─── MAIN ────────────────────────────────────────────────────────────────────

st.set_page_config(
    page_title="IMPOWER Gait Analysis System",
    page_icon="👣",
    layout="wide",
...
def main():
    ...
    if not st.session_state.logged_in:
        st.markdown("""
        <style>
            .login-title { font-size:2.8em; color:#37474f; text-align:center; font-weight:bold; margin-bottom:0.5rem; }
            .login-sub   { font-size:1.1em; color:#607d8b; text-align:center; margin-bottom:2rem; }
        </style>
        """, unsafe_allow_html=True)
        st.markdown('<div class="login-title">👣 IMPOWER Gait Analysis</div>', unsafe_allow_html=True)
        st.markdown('<div class="login-sub">Please log in to continue</div>', unsafe_allow_html=True)

        login_col1, login_col2, login_col3 = st.columns([1, 2, 1])
        with login_col2:
            st.subheader("Login")
            user_mode = st.radio("Select your role:", ["Administrator", "Patient"], horizontal=True)
    ...
    st.markdown(
        f'<div style="font-size:2.4em; color:{header_color}; text-align:center; font-weight:bold; margin-bottom:0.5rem;">👣 IMPOWER Gait Analysis</div>',
        unsafe_allow_html=True)
    ...
    if st.session_state.user_mode == "Administrator":
        ...
        with st.sidebar.expander("Patient Profile"):
            ...
            st.text_input("Month/Year of Birth", key="patient_dob")
        ...
        with st.sidebar.expander("My Profile"):
            st.text_input("Sex", key="pat_sex")
            st.text_input("Date of Birth", key="pat_dob")
            st.number_input("Height (cm)", key="pat_height")
            st.number_input("Weight (kg)", key="pat_weight")
        patient_id = "USER-" + st.session_state.user_email[:4].upper()
    ...
    available_metrics = ["Cadence", "Step Length", "Stride Length", "Step Time",
                         "Gait Speed", "Swing Time", "Stance Time", "Symmetry"]
    ...
    if df is not None:
        ...
        if st.session_state.user_mode == "Administrator":
            render_admin_dashboard(df, stats, session_time, total_distance,
                                   target_cadence_min, target_cadence_max,
                                   target_step_length_min, target_step_length_max,
                                   filtered_df)
        else:
            render_patient_dashboard(df, stats, session_time, total_distance,
                                     target_cadence_min, target_cadence_max,
                                     target_step_length_min, target_step_length_max,
                                     filtered_df)
    else:
        if st.session_state.user_mode == "Patient":
            st.info("👈 Upload your session CSV file from the sidebar to get started!")
        else:
            st.info("👈 Upload a session CSV file from the sidebar to begin analysis.")
